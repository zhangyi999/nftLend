...
contract MockUniswapV2Router02 is MockUniswapV2Router02IUniswapV2Router02 {
  ...
  modifier ensure(uint deadline) {
    require(deadline >= block.timestamp, 'MisRouter: EXPIRED');
    _;
  }
  ...
  function _addLiquidity(
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, 'MisRouter: INSUFFICIENT_B_AMOUNT');
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint amountAOptimal =
          MockUniswapV2Router02UniswapV2Library.quote(amountBDesired, reserveB, reserveA);
        assert(amountAOptimal <= amountADesired);
        require(amountAOptimal >= amountAMin, 'MisRouter: INSUFFICIENT_A_AMOUNT');
  ...
  function removeLiquidity(
    ...
    require(amountA >= amountAMin, 'MisRouter: INSUFFICIENT_A_AMOUNT');
    require(amountB >= amountBMin, 'MisRouter: INSUFFICIENT_B_AMOUNT');
  }
  ...
  function swapExactTokensForTokens(
    ...
    require(
      ...
      'MisRouter: INSUFFICIENT_OUTPUT_AMOUNT'
    );
  ...
  function swapTokensForExactTokens(
    ...
    require(amounts[0] <= amountInMax, 'MisRouter: EXCESSIVE_INPUT_AMOUNT');
    MockUniswapV2Router02TransferHelper.safeTransferFrom(
      path[0],
      msg.sender,
      MockUniswapV2Router02UniswapV2Library.pairFor(factory, path[0], path[1]),
  ...
  function swapExactETHForTokens(
    ...
    require(path[0] == WETH, 'MisRouter: INVALID_PATH');
  ...
  function swapTokensForExactETH(
    ...
    require(path[path.length - 1] == WETH, 'MisRouter: INVALID_PATH');
    amounts = MockUniswapV2Router02UniswapV2Library.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= amountInMax, 'MisRouter: EXCESSIVE_INPUT_AMOUNT');
    MockUniswapV2Router02TransferHelper.safeTransferFrom(
      path[0],
  ...
  function swapETHForExactTokens(
    ...
    require(path[0] == WETH, 'MisRouter: INVALID_PATH');
    amounts = MockUniswapV2Router02UniswapV2Library.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= msg.value, 'MisRouter: EXCESSIVE_INPUT_AMOUNT');
    MockUniswapV2Router02IWETH(WETH).deposit{value: amounts[0]}();
    assert(
      MockUniswapV2Router02IWETH(WETH).transfer(
        MockUniswapV2Router02UniswapV2Library.pairFor(factory, path[0], path[1]),
  ...
  function swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    require(amountOut >= amountOutMin, 'MisRouter: INSUFFICIENT_OUTPUT_AMOUNT');
