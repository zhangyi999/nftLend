...
contract UniswapV2ERC20 {
  ...
  function permit(
    ...
    require(deadline >= block.timestamp, 'MisPair: EXPIRED');
    bytes32 digest =
      keccak256(
        abi.encodePacked(
          '\x19\x01',
...
contract MockUniswapV2FactoryUniswapV2Pair is UniswapV2ERC20 {
  ...
  modifier lock() {
    require(unlocked == 1, 'MisPair: LOCKED');
    unlocked = 0;
    _;
    unlocked = 1;
  }
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory, 'MisPair: FORBIDDEN'); // sufficient check
    token0 = _token0;
    token1 = _token1;
  }
  ...
  function _update(
    ...
    require(balance0 <= MAX1 && balance1 <= MAX1, 'MisPair: OVERFLOW');
  ...
  function mint(address to) external lock returns (uint liquidity) {
    ...
    require(liquidity > 0, 'MisPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'MisPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function swap(
    ...
    require(amount0Out > 0 || amount1Out > 0, 'MisPair: INSUFFICIENT_OUTPUT_AMOUNT');
    (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'MisPair: INSUFFICIENT_LIQUIDITY');
    ...
    uint balance1;
    {
      ...
      require(to != _token0 && to != _token1, 'MisPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'MisPair: INSUFFICIENT_INPUT_AMOUNT');
    {
...
contract MockUniswapV2Factory {
  ...
  function createPair(address tokenA, address tokenB) external returns (address pair) {
    require(tokenA != tokenB, 'MisPair: IDENTICAL_ADDRESSES');
    (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'MisPair: ZERO_ADDRESS');
    require(getPair[token0][token1] == address(0), 'MisPair: PAIR_EXISTS'); // single check is sufficient
  ...
  function setFeeTo(address _feeTo) external {
    require(msg.sender == feeToSetter, 'MisPair: FORBIDDEN');
    feeTo = _feeTo;
  }
